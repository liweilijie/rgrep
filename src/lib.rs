...
pub type StragegyFn<W, R> = fn(&Path, BufReader<R>, &Regex, &mut W) -> Result<(), GrepError>;

/// 简化版本的 grep , 支持正则表达式和文件通配符
#[derive(Parser, Debug)]
#[clap(version = "1.0", author = "liwei")]
...
impl GrepConfig {
    ...
    /// 使用某个策略函数来查找匹配
    pub fn match_with(&self, strategy: StragegyFn<Stdout, File>) -> Result<(), GrepError> {
...
mod tests {
    ...
    fn default_strategy_should_work() {
        ...
        default_strategy(path, reader, &pattern, &mut write).unwrap();
        let result = String::from_utf8(write).unwrap();
        let expected = [
            String::from("src/main.rs"),
            format_line("hello world!", 1, 0..5),
            format_line("hey Liwei!\n", 2, 0..3),
        ];
